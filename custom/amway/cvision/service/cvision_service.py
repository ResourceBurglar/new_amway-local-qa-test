...
from service.local_entity_service import ChatBotDomain, NamespaceDomain, NamespaceFileDomain
...
class CvDomain(BaseComposeBot, BaseChatMessage):
    ...
    def ask(
        ...
        # 根据标识查询机器人配置信息
        chatBotDomain = ChatBotDomain(request_id=self.request_id)
    ...
    def ask_reload(
        ...
        while len(data) < num:
            ...
            for k, v in slave_bot_dict.items():
                ...
                if _bot_model:
                    _answer = chatPrivateDomain.ask_to_docs(
                        ques_docs=[split_docs_tuple[0]],
                        ques=_bot_model.fixed_ques,
                        prompt=_bot_model.prompt,
                        prompt_variables=_bot_model.prompt_variables,
                        split_chunk_type=_bot_model.chains_chunk_type,
                        prefix_prompt=_bot_model.prefix_prompt,
                        prefix_prompt_variables=_bot_model.prefix_prompt_variables,
                        llm=LLMsAdapter(model=CVISION_CHOOSE_LLM).get_model_instance()
                    )
