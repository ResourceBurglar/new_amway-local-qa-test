...
from service.local_entity_service import NamespaceDomain, NamespaceFileDomain
...
class NamespacePrepareService:
    ...
    def handle(self):
        # 查询指定知识库
        namespaceModel = NamespaceDomain(request_id=self.request_id).find_by_id(namespace_id=self.namespace_id)
        ...
        # 查询知识库所属文件清单
        file_list = NamespaceFileDomain(request_id=self.request_id).find_by_condition(namespace_id=self.namespace_id)
        # 处理需要删除的向量数据
        if self.param.delete_list and len(self.param.delete_list) > 0:
            ...
            for namespaceFileModel in file_list:
                ...
                # 更新业务数据
                NamespaceFileDomain(request_id=self.request_id).update(
        ...
        if self.param.insert_list and len(self.param.insert_list) > 0:
            ...
            # 保存向量数据
            ids = get_instance_client().insert_data(
                split_docs=docs,
    ...
    def handle_by_file_id(self, namespaceModel: NamespaceModel):
        ...
        namespaceFileModel = NamespaceFileDomain(request_id=self.request_id).find_by_id(file_id=self.file_id)
